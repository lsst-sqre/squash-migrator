...
class Transformer(Actuator):
    ...
    def transform(self):
        ...
        for inp_file in inputfiles:
            ...
            with open(out_file, "w") as f:
                self.logger.debug("Writing job to file '%s'." % out_file)
                json.dump(transformed_job, f, sort_keys=True, indent=4)
            so_far = so_far + 1
    ...
    def _get_metric_name(self, metric, metadata):
        ...
        if metadata:
            ...
            if spec_name:
                newname += "_" + spec_name
            filter = metadata.get("filter_name")
            if filter:
                newname += "_" + filter
        metric_tuple = (metric, spec_name, filter)
